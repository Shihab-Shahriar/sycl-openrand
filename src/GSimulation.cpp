...
//using RNG = oneapi::dpl::philox4x32x10<2>;
...
void GSimulation::InitPos() {
  ...
  for (int i = 0; i < get_npart(); ++i) {
    RNG rng(i, 0);
    particles_[i].pos[0] = rng.rand();
    particles_[i].pos[1] = rng.rand();
    particles_[i].pos[2] = rng.rand();
  }
}
...
void GSimulation::InitVel() {
  ...
  for (int i = 0; i < get_npart(); ++i) {
    ...
    particles_[i].vel[0] = rng.rand() * 1.0e-3f;
    particles_[i].vel[1] = rng.rand() * 1.0e-3f;
    particles_[i].vel[2] = rng.rand() * 1.0e-3f;
  }
}
